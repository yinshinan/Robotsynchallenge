...
class ItemAssemblyEnv(EmbodiedEnv):
    ...
    def create_demo_action_list(self, *args, **kwargs):
        ...
        self._guijiao_constraint_check_completed = False
        if (
    ...
    def _attach_guijiao_if_aligned(self):
        ...
        if getattr(self, "_guijiao_constraint_check_completed", False):
            return

        self._guijiao_constraint_check_completed = True
        ...
        task_ready = self.is_task_success()
        ready_to_attach = aligned & task_ready
        ...
        logger.log_info(
            ...
            f"task_ready={task_ready.detach().cpu().tolist()}, "
            f"ready_to_attach={ready_to_attach.detach().cpu().tolist()}"
        )

        if not bool(ready_to_attach.all().item()):
            return
    ...
    def is_task_success(self, **kwargs) -> torch.Tensor:
        ...
        if getattr(self, "_guijiao_constraint_created", False):
            ...
            self._guijiao_constraint_created = False
            logger.log_info(
                "[ItemAssembly] Removed guijiao constraint after final check."
            )
