...
def main():
        ...
        for episode in range(max_episodes):
            ...
            try:
                ...
                while env_steps < max_env_steps:
                    ...
                    policy_action_steps = info.get("_policy_action_steps")
                    if policy_action_steps is not None:
                        env_steps += int(policy_action_steps)
                    else:
                    ...
                    if hasattr(info_success, "any"):
                        info_success = bool(info_success.any().item())
                    else:
                        info_success = bool(info_success)

                    task_success = env.get_wrapper_attr(
                        "is_task_success"
                    )()
                    if hasattr(task_success, "any"):
                        task_success = bool(task_success.any().item())
                    else:
                        task_success = bool(task_success)

                    if info_success or task_success:
                        episode_success = True
                        progress_bar.write("Task success!")
                        break
                    if truncated:
                        progress_bar.write("Task terminated or timeout!")
                        break
            finally:
                if progress_bar:
                    progress_bar.close()
            ...
            def _metric_bool(name, default=False):
                value = info.get("metrics", {}).get(name, default)
            ...
            if not episode_success:
                ...
                elif not _metric_bool("held"):
                    failure_reason = "Bottle was not held"
                elif not _metric_bool("pouring_started"):
                    failure_reason = "Valid pouring was never started"
                elif not _metric_bool("relative_pose_valid"):
                    failure_reason = "Bottle mouth was not aligned with the cup"
                elif _metric_bool("pouring_now"):
                    failure_reason = "Bottle remained in the pouring state"
                elif not _metric_bool("returned_upright"):
                    failure_reason = "Bottle was not returned upright"
                elif env_steps >= max_env_steps:
                    failure_reason = "Maximum action steps reached"
                else:
                    failure_reason = "Task success conditions were not satisfied"

            avg_policy_eval_time = (
                float(np.mean(episode_policy_eval_times))
                if episode_policy_eval_times
                else 0.0
            )
            total_episode_inference_time = float(
                np.sum(episode_policy_eval_times)
            )
            total_action_steps += env_steps
            all_inference_times.extend(episode_policy_eval_times)
            episode_results.append(
              {
                "episode": episode + 1,
                "success": episode_success,
                "action_steps": env_steps,
                "inference_calls": len(episode_policy_eval_times),
                "avg_inference_time": avg_policy_eval_time,
                "total_inference_time": total_episode_inference_time,
            ...
            print(f"  Episode {episode+1:02d}/{max_episodes:02d}: {status}")
            if not episode_success:
                print(f"  Failure Reason: {failure_reason}")
            print(
                f"  Success Rate: {success_count}/{episode+1} "
                f"({100*success_count/(episode+1):.2f}%)"
            )
            print(f"  Action Steps: {env_steps}")
            print(
                f"  Inference Time: avg={avg_policy_eval_time*1000:.2f} ms/call, "
                f"total={total_episode_inference_time:.3f} s, "
                f"calls={len(episode_policy_eval_times)}"
            )
    finally:
        if video_recorder:
    ...
    overall_avg_inference_time = (
        float(np.mean(all_inference_times))
        if all_inference_times
        else 0.0
    )
    avg_action_steps = (
        total_action_steps / max_episodes
        if max_episodes
        else 0.0
    )
    ...
    print(f"{'='*72}")
    print(
        f"  Success Rate: {success_count}/{max_episodes} "
        f"({100 * success_count / max_episodes:.1f}%)"
    )
    print(f"  Total Action Steps: {total_action_steps}")
    print(f"  Average Action Steps: {avg_action_steps:.2f} steps/episode")
    print(
        f"  Average Inference Time: "
        f"{overall_avg_inference_time:.6f} s/call "
        f"({overall_avg_inference_time * 1000:.2f} ms/call)"
    )
    ...
    print("\n  Per-Episode Results")
    print(
        "  Episode | Result  | Action Steps | "
        "Inference Calls | Avg Inference Time"
    )
    print(f"  {'-'*66}")
    ...
    for result in episode_results:
        ...
        print(
            f"  {result['episode']:7d} | "
            f"{status:7s} | "
            f"{result['action_steps']:12d} | "
            f"{result['inference_calls']:15d} | "
            f"{result['avg_inference_time'] * 1000:17.2f} ms"
        )    
